...
| ------------------ | ---------- | ------------------------------- | -------- | ---------- |
| Type 1 LSA——Router | 和右边一样 | 设备自己的RID（谁发的就是谁的） | 单区域   | 拓扑信息   |
...
| 类别                | LS ID      | 通告路由器          | 传播范围 | 携带的数据         |
| ------------------- | ---------- | ------------------- | -------- | ------------------ |
| Type-2 LSA——network | DR接口的IP | MA网络中DR设备的RID | 单区域   | 拓扑信息的补充信息 |
...
| 类别                           | LS ID                | 通告路由器 | 传播范围 | 携带的数据 |
| ------------------------------ | -------------------- | ---------- | -------- | ---------- |
| Type-3 LSA——Sum-Net（summary） | 传递的本条路由的网段 | ABR设备    |          |            |
...
Type-3 LSA——Sum-Net————都是由ABR设备进行通告，使用传递的这条路由的目标网段作为LS ID，里面有路由的子网掩码和开销值，LSA中携带的开销值为通告者到达目标网段的开销值，收到该LSA时需要对将该路由加入到路由表中，开销值就是自己到通告者再加上原来的，加到路由表上时，从哪个接口收到这个消息的，就把这个接口写路由表上

...
<img src="C:\Users\xgz24\AppData\Roaming\Typora\typora-user-images\image-20260921103339799.png" alt="image-20260921103339799" style="zoom:50%;" />

这里面多了VEB这三个标记位，

<img src="C:\Users\xgz24\AppData\Roaming\Typora\typora-user-images\image-20260921103558325.png" alt="image-20260921103558325" style="zoom:50%;" />

V————如果V位置1，则代表该设备是虚链路的一个端点

E————如果该位置1，则代表该设备是ASBR设备

B————如果该位置1，则代表该设备是ABR设备
...
*   Metric度量值（COST）—— 因为不同协议开销值的计算规则不同，所以，重发布之后，将舍弃原先网络的开销值，并赋予一个初始的种子度量值（seed-metric）。OSPF默认的种子度量值为1。
    ...
    重发布到RIP中的路由种子度量值设定为0。

`[r4-ospf-1]import-route rip 1 cost 2`重发布的时候修改度量值



*   E Type——开销值类型类型——类型1——————E位置0————所有设备到达域外该路由的开销值为到达通告者沿途开销值的累加值加种子度量值（到通告者开销值+种子度量值）
    						类型2（默认）——E位置1————所有设备到达域外该路由的开销值均为种子度量值（直接设置为种子度量值）

    `[r4-ospf-1]import-route rip 1 type 1`重发布的时候修改开销值类型

*   ForwardingAddress————转发地址————看第六个LSA
...
| 类别 | LS ID                | 通告路由器 | 传播范围   | 携带的数据       |
| ---- | -------------------- | ---------- | ---------- | ---------------- |
| NSSA | 传递的本条路由的网段 | ASBR设备   | NSSA单区域 | 路由信息（域外） |

还有7类LSA，其实就是5类的套壳，为了防止优化的特殊区域把他当5类一样杀了

好吧，这个就是第六种，可以去看OSPF优化方案的特殊区域

<img src="C:\Users\xgz24\AppData\Roaming\Typora\typora-user-images\image-20260920231108160.png" alt="image-20260920231108160" style="zoom:67%;" />

E一般情况下都会置1（显示出来就是置1）——代表支持5类LSA

N---只有在NSSA区域置1--代表支持7类LSA
...
转发地址——在OSPF中如果出现选路不佳的情况，则可以将最佳的下一跳放在转发地址中，设备将按照最佳的选路转发。（如果FA地址中存在信息，则将直接按照FA地址的信息找下一跳，而不再找通告者）

设备看到转发地址，会先看看能不能达到，能达到，就直接去找转发地址



如果不存在选路不佳（没人知道，所谓的检测到选路不佳就是没有环回接口，仅此而已），5类LSA中会使用0.0.0.0来进行填充，7类LSA中会使用ASBR所有环回地址中最先宣告的地址作为转发地址

​	规则 1：如果 OSPF 区域内配置了 Loopback 接口，就直接选 Loopback 的 IP 地址作为 FA。

​	规则 2：如果没有 Loopback，就选 OSPF 接口列表中“第一个”接口的 IP 地址。

​	规则 3：以上条件都不满足（比如接口不是广播类型、接口没使能 OSPF 等），FA 就填 0.0.0.0。
...
​	1，汇总——减少骨干区域的更新量（对一个区域进行汇总后交给骨干区域）
...
<img src="C:\Users\xgz24\AppData\Roaming\Typora\typora-user-images\image-20260920194734604.png" alt="image-20260920194734604" style="zoom:67%;" />

```
[r1-ospf-1-area-0.0.0.2]abr-summary 192.168.0.0 255.255.252.0

要对哪个区域进行汇总就在哪个区域敲
敲完之后除这个区域以外的区域就可以收到汇总后的路由了，这个区域外的路由表会得到精简
```
...
因为这是根据3类LSA进行汇总，所以去左边1.0和左边2.0需要用一类二类构建拓扑信息的ABR的路由表并不会汇总。（就是说的R1）





注意：ABR设备只能将自己通过1类和2类LSA学习到的路由转化成3类进行汇总（就是没办法跨区域汇总）

为什么？因为：

​	1，3类LSA已经是区域间的路由摘要了，它代表的是别的区域的汇总结果。

​	2，如果你再汇总一次，相当于对“别人的汇总”再汇总，信息会失真，而且容易出环路。

​	3，OSPF 的区域分层设计就是：每个区域内部自己管明细，ABR 只负责把本区域的明细转成 3类发出去。不能反过来把从别处学来的 3类再加工。
...
<img src="C:\Users\xgz24\AppData\Roaming\Typora\typora-user-images\image-20260920194724623.png" alt="image-20260920194724623" style="zoom:67%;" />

```
[r4-ospf-1]asbr-summary 172.16.0.0 255.255.252.0

在要传进来的ASBR设备上敲就行，会把外面的RIP进行汇总，OSPF里的路由表得到精简
```



注意：如果开销值类型为类型1，则汇总之后的度量值为所有明细路由度量值中的最大值。

​	  如果开销值类型为类型2，则汇总之后的度量值为所有明细路由度量值中的最大值加1。
...
————如果将一个区域配置成为末梢区域，则效果是改区域设备将不再学习4类和5类LSA，自动生成一条指向骨干的3类缺省。

```
[r5-ospf-1-area-0.0.0.2]stub
将区域2设定为末梢区域（整个区域里面所有设备都需要配置相同设置）
```

注意：如果将一个区域配置成为末梢区域，则改区域内所有设备都需要完成相同的设置

<img src="C:\Users\xgz24\AppData\Roaming\Typora\typora-user-images\image-20260920204532155.png" alt="image-20260920204532155" style="zoom: 67%;" />
...
2，完全末梢区域（Totally stub）————拒绝学习4类和5类LSA，并且拒绝学习3类LSA，但是保留3类缺省

```
[rl-ospf-1-area-0.0.0.2] stub no-summary
只在ABR设备这一个上面敲就好，毕竟区域里面的都是ABR转发过去的
```
...
第二类特殊区域（NSSA）————1，不能是骨干区域；（就是末端后面还挂着个别的配置）
...
1，非完全末梢区域（NSSA）————如果将一个区域配置成为NSSA区域，则该区域会拒绝学习4类和5类LSA。自动生戒一条指向骨干的7类缺省
...
整个区域每个设备都要敲（将区域1设置为非完全末梢区域）
```



那他都拒绝5类了，该怎么把域外信息传递过去呢，这个时候就需要7类这个套壳上场了，其实就是伪装五类，这样一系列操作下来的结果就是可以NSSA 的 ABR（同时连接区域 0）在将 7 类 LSA 转译为 5 类 LSA 时，从骨干区域的角度看，这个ABR充当了 ASBR 的角色。，从而让骨干区域没有4类！！🧐🤪有人可能会问这有什么用，咱们换个场景，要是有还有一个RIP挂在另一个区域呢，这样是不是就不用学习那个RIP的5类和4类了，这就是原因

<img src="C:\Users\xgz24\AppData\Roaming\Typora\typora-user-images\image-20260920230309502.png" alt="image-20260920230309502" style="zoom:67%;" />
...
————可以在拒绝学习4类和5类LSA的基础上，进一步拒绝学习3类LSA，并且，生成一条指向骨干的三类缺省。

​			(3类缺省和7类缺省会同时存在，但是，最终将选择3类缺省（3类优于7类)）

```
[r3-ospf-1-area-0.0.0.1]nssa no-summary
敲在ABR设备上

[R3-ospf-1-area-0.0.0.1]nssa translator-always
让这位ABR成为转化者，防止出现以下情况
```



OSPF 规定：**NSSA 区域里有多个 ABR 时，Router ID 最大的那个 ABR 自动成为转换者。**

你的 Area 3 里有两个 ABR：

| 路由器 | Router ID | 是否连 Area 0 | 转换者选举结果 |
| :----- | :-------- | :------------ | :------------- |
| R6     | 6.6.6.6   | 连            | 输             |
| R7     | 7.7.7.7   | 不连          | 赢             |

R7 的 Router ID 更大，所以 OSPF 选了 R7 当转换者。但是我们意愿并非如此，所以需要手动指定转换者
...
<img src="C:\Users\xgz24\AppData\Roaming\Typora\typora-user-images\image-20260922085838381.png" alt="image-20260922085838381" style="zoom:67%;" />
...
可以修改类型字段为ospf路由的默认优先级
```

<img src="C:\Users\xgz24\AppData\Roaming\Typora\typora-user-images\image-20260922084454528.png" alt="image-20260922084454528" style="zoom:67%;" />
...
修改类型字段为O_ASE和O_NSSA路由的默认优先级
```

<img src="C:\Users\xgz24\AppData\Roaming\Typora\typora-user-images\image-20260922084838339.png" alt="image-20260922084838339" style="zoom:67%;" />



这种办法只能控制不同路由协议之间的选择，同一种协议下无法控制
...
```
[R5-ospf-1]bandwidth-reference 1000
修改参考带宽，OSPF域内的所有设备都必须修改为相同的参考带宽
```
...
```
[R5-GigabitEthernet0/0/0]undo negotiation auto 
关闭接口的自动协商

[R5-GigabitEthernet0/0/0]speed 100
修改真实带宽

需要修改真实带宽，而且只能改小不能改大，导致传输速率降低，不推荐
```

两边接口开销值不一样也没事，算路由表开销值时算的是从要到达的地方到自己所会经历的入接口的开销值的相加
...
注意：控制层面的流量和数据层面的流量方向正好相反（上面的算路由表开销值很明显用的是控制层面的流量）

注意：OSPF开销值的计算规则为控制层流量流入方向接口开销值的累加值或者数据层流量流出方向的接口开销值的累加值
...
```
[R5-GigabitEthernet0/0/1]ospf cost 100
直接修改接口开销值（这个也能修改环回接口开销值）
```
...
<img src="C:\Users\xgz24\AppData\Roaming\Typora\typora-user-images\image-20260922095530558.png" alt="image-20260922095530558" style="zoom:80%;" />

首先先通过一个场景来引入，R3对左边的区域1发送三类LSA给区域0时，我们知道，三元确定一个LSA，R1和R2网段的LS ID都是20.1.0.0（LS ID是目标网段的网络号），类型一样，通告者都是R3的3.3.3.3，完了，这不一样了吗，五类和七类和这个一样的道理，这个附录E就是为了解决这个问题



在以上这种情况下，掩码较短的LSA正常传递，掩码较长的LSA将LS D改成他的直接广播地址再传递。

举个例子，20.1.0.0/16正常传20.1.0.0，20.1.0.0/24传20.1.0.255
...
**1，域内和域内**————比较开销值，优先选择开销值小的，如果开销值相同，则将负载均衡。

<img src="C:\Users\xgz24\AppData\Roaming\Typora\typora-user-images\image-20260922105256154.png" alt="image-20260922105256154" style="zoom:33%;" />
...
**1，域间和域间**————比较开销值，优先选择开销值小的，如果开销值相同，则将负载均衡。

<img src="C:\Users\xgz24\AppData\Roaming\Typora\typora-user-images\image-20260922105221028.png" alt="image-20260922105221028" style="zoom:33%;" />



**3，域外和域外**

​	开销值类型二比较规则：优先看种子度量值，优先选择种子度量值小的路径，若种子度量值相同，则选择沿途累加值较小的，若沿途累加值也相同，则负载均衡（先看种子度量值，再看沿途累加值）



​	类型一永远优于类型二（一条类型二，一条类型一，直接选开销值为类型一的路线）



​	开销值类型一比较规则：直接比较种子度量值和沿途累加值的总和，优先选择总和小的路径，如果总和相同，则负载均衡





**4，域内和域间**————无条件走域内那条路，不看开销值



<img src="C:\Users\xgz24\AppData\Roaming\Typora\typora-user-images\image-20260922150800121.png" alt="image-20260922150800121" style="zoom:33%;" />



**5，域间和域外**————无条件走域间那条路，不看开销值



<img src="C:\Users\xgz24\AppData\Roaming\Typora\typora-user-images\image-20260922152054724.png" alt="image-20260922152054724" style="zoom:33%;" />
...
<img src="C:\Users\xgz24\AppData\Roaming\Typora\typora-user-images\image-20260923231521825.png" alt="image-20260923231521825" style="zoom:80%;" />

做综合实验遇到一个好玩的，R11宣告的172.16.0.0的RIP，导致他把0/0/1接口也算到RIP里面了，从R5会看到172.16.66.0/24在外面

<img src="C:\Users\xgz24\AppData\Roaming\Typora\typora-user-images\image-20260923231712072.png" alt="image-20260923231712072" style="zoom:67%;" />

但在给区域2做了NSSA之后，这条LSA就消失了，因为原本区域2汇总后的网段是由R9发送的，而这条66是由R11发送的，所以会认为是两条，但是在做完NSSA之后，R11需要给R9发那条66做7转5操作，现在就变成了汇总网段和66都是R9发送的，这两条就合并了
